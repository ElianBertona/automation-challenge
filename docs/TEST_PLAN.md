...
- **Targeted CRUD Flow:** Automating the _Create -> Verify -> Edit -> Verify_ sequence using unique identifiers (`Date.now()`) to isolate session data.
...
- **Risk:** XPath selectors break due to UI changes.
  - _Mitigation:_ Use of relative XPaths and `ancestor::` axis in Page Objects.
- **Risk:** Shared Environment Data (Other users editing our items).
  - _Mitigation:_ Tests use high-precision unique strings to interact only with session-specific data.
- **Risk:** Heroku Latency (Timeouts).
  - _Mitigation:_ `waitforTimeout` increased to 10 seconds in `wdio.conf.js`.
